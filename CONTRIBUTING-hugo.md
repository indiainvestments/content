...
assets/css/main.css   the stylesheet (plain CSS, gruvbox palette)
...
The palette is gruvbox, defined once as custom properties at the top of
`main.css`: a `:root` block for light, then the same tokens redefined under
...
from them. The toggle in the top bar writes the choice to `localStorage`, and a
